...
public enum SwarmToolRegistryError: Error, Equatable, Sendable {
...
public struct SwarmToolRegistry: HiveToolRegistry, Sendable {
    ...
    public static func fromRegistry(_ registry: ToolRegistry) async throws -> Self {
        ...
        let definitions = try schemas
            ...
            .sorted { $0.name.utf8.lexicographicallyPrecedes($1.name.utf8) }
        return SwarmToolRegistry(registry: registry, toolDefinitions: definitions)
    }
...
extension SwarmToolRegistry {
    ...
    private static func encodeJSONFragment(_ value: SendableValue) throws -> String {
        ...
        guard let json = String(data: data, encoding: .utf8) else {
            throw SwarmToolRegistryError.resultEncodingFailed
        }
        return json
    }
    ...
    private static func makeToolDefinition(for schema: ToolSchema) throws -> HiveToolDefinition {
        ...
        guard let json = String(data: data, encoding: .utf8) else {
            throw SwarmToolRegistryError.schemaEncodingFailed
        }
