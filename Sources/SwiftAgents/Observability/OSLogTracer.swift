...
public actor OSLogTracer: AgentTracer {
    ...
    private func handleSignpostEvent(_ event: TraceEvent) {
        ...
        case .agentStart, .toolCall:
            startSignpostInterval(for: event)

        // End signpost intervals
        case .agentComplete, .agentError, .agentCancelled, .toolResult, .toolError:
    ...
    private func endSignpostInterval(for event: TraceEvent) {
        ...
        switch event.kind {
        case .agentComplete, .agentError, .agentCancelled:
            name = "Agent Execution"
