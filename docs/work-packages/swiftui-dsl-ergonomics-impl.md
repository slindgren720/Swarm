...
Task Breakdown:
- Update Router handling in `Sources/SwiftAgents/Orchestration/OrchestrationBuilder.swift`:
...
- (Optional) Add `When(..., use:)` and `Otherwise(use:)` overloads in `Sources/SwiftAgents/DSL/Flow/Routes.swift`.
Expected Output:
