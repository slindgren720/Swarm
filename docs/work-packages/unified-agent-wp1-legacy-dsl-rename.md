...
0) Preflight: identify all module-level `Agent` declarations that would collide with a future `public actor Agent`:
   ...
   - Also check for other top-level declarations named `Agent` (not part of this plan doc, but can block compilation), e.g.:
     - `public macro Agent(...)` in `Sources/Swarm/Macros/MacroDeclarations.swift`
...
2) Rename the legacy DSL protocol and update all constraints/overloads:
   - Edit `Sources/Swarm/DSL/DeclarativeAgent.swift`:
   ...
   - Edit `Sources/Swarm/DSL/LoopAgent.swift`:
   ...
   - Edit `Sources/Swarm/DSL/AgentLoopBuilder.swift`:
...
3) Deprecations + migration messages (must be precise and actionable):
   ...
   - Deprecate the legacy flow steps:
     - Edit `Sources/Swarm/DSL/Flow/Generate.swift` and `Sources/Swarm/DSL/Flow/Relay.swift`.
...
4) Update modifiers/adapters that wrap legacy DSL:
   - Inspect and update:
     ...
     - `Sources/Swarm/DSL/Modifiers/EnvironmentAgent.swift`
     - Any other DSL utilities found via `rg -n \"\\bAgent\\b\" Sources/Swarm/DSL` that are talking about the legacy protocol, not `AgentRuntime`.
5) Update tests to compile with the rename:
   ...
   - Likely files:
     ...
     - `Tests/SwarmTests/Orchestration/HandoffConfigurationTests+Builder.swift`
